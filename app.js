...
client.on('message', message => {
  ...
  if (message.channel.id === "361527984756097024" && message.content === '@everyone') {
  message.react("427907939810934807").then(() => message.react('♥')).then(() => message.react('👌')).then(() => message.react('✅')).then(() => message.react('😄'));
};
});
client.on('message', (message) => {
...
if (message.author.id === '237171563760320514') return;
if (message.author.id === '276715825262755842') return;
if (message.author.id === '154924680770355200') return;
if (message.author.id === '240176275753271306') return;
if (message.author.bot) return;
...
if (foundInText) {
	message.delete();
	message.channel.send(`Sorry ${message.author.username}, but you are not allowed to say that.`)
  ...
  if (!sayMessage)
    ...
    const embed = new Discord.RichEmbed()
    .setAuthor(`${message.author.username} suggests:`, `${message.author.avatarURL}`)
    .setColor(0x00AE86)
